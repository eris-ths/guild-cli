...
test('devil schema --format json emits the v0 contract (open + entry + list + show + conclude + schema)', (t) => {
  const { root, cleanup } = tmpRoot();
  t.after(cleanup);
  ...
  // Currently implemented: open + entry + list + show + conclude +
  // schema. Subsequent commits add ingest / dismiss / resolve /
  // suspend / resume.
  const names = payload.verbs.map((v: { name: string }) => v.name).sort();
  assert.deepEqual(
    names,
    ['conclude', 'entry', 'list', 'open', 'schema', 'show'],
  );
});
...
test('devil schema --format text emits a terse summary', (t) => {
  ...
  assert.equal(r.status, 0);
  assert.match(r.stdout, /devil-review/);
  assert.match(r.stdout, /6 verb\(s\)/);
