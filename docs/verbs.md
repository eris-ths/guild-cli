...
empty) still records but is tagged `okf:none` — so a stray non-concept
`.md` (a README, a note) is auditable via `ctx list --tag okf:none`
(the marker is `none`, not `untyped`, so it can't be confused with a real
type literally named "Untyped") rather than passing silently as a Fact;
documents lacking an author fall
